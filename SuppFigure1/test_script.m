...
print(gcf,'-dpdf',[filepathanalysis, 'SupplFigure1C_deltavelociy_13mice_averaged.pdf'])

saveas(gcf, [filepathanalysis, 'SupplFigure1C_deltavelociy_13mice_averaged.pdf']);
...
set(gcf,'paperunits','centimeters','papersize' ,[21,29.7],'color','w','paperposition',[0,0,21,29.7],'inverthardcopy','off');
filepathanalysis=['G:\mousebox\code\mouselab\users\karolina\FiguresPaper2023\SuppFigure1\'];
%print(gcf,'-dpdf',[filepathanalysis, 'average_raw_vel_relative_delta_velocity_SEM_sessions.pdf']);
%% FIGURE 1D %% scatter plot COLOR CODED STIMULUS PAIR
order_nasal
order_temporal
...
for iStim=1:6
    ...
    clear index_inf_nasal
    clear index_inf_temporal
    index_inf_nasal=find(data_nasal==Inf);
    index_inf_temporal=find(data_temporal==Inf);
    
    if sum([length(index_inf_nasal), length(index_inf_temporal)])>0
        data_nasal(index_inf_nasal)=0;
        data_temporal(index_inf_temporal)=0;
        sprintf('Inf trials')
    end
    hold on
%   plot(100*data_temporal(:),100*data_nasal(:),'o','MarkerSize', 6,'MarkerFaceColor', [0.7,0.7,0.7],'MarkerEdgeColor', [0.5, 0.5, 0.5]); %colors_scatter(iSess,:)
...
%print(gcf,'-dpdf',[filepathanalysis, 'average_delta_velocity_SEM_animals.pdf']);
