...
diam_av_interpolated{iAn}=nanmean(diam_interpolated{iAn},2)
...
%%
%% random experiments
for iAn=1:size(diam_epochs_random,2)
clear rand_diam
...
%
nstims=size(diam_epochs_random,3)
nAn=size(diam_epochs_random,2)
...
for iAn=1:size(diam_epochs_non_random,2)
clear nonrand_diam
...
nAn=size(diam_epochs_non_random,2)
...
%
nAn=size(diam_epochs_non_random,2)
nstims=size(diam_epochs_non_random,3)
...
for iStim=1:12
ax_avpupil_stims_random(iStim) = axes('position',[.045+.08*(iStim-1),.2,.05,.1],'units','normalized');
ax_avpupil_stims_nonrandom(iStim) = axes('position',[.045+.08*(iStim-1),.35,.05,.1],'units','normalized');
end
...
for iStim=1:nstims-1
axes(ax_avpupil_stims_nonrandom(iStim));
hold all
clear x
clear y
clear errBar
...
title(sprintf('%d deg',stimulus(iStim)))
if iStim==1
   
set(ax_avpupil_stims_nonrandom(iStim),'box','off','tickdir','out','xtick',[cframespre cframesstim],...
   'xticklabel',[0 5],'XColor', 'none','ticklength',get(ax_avpupil_stims_nonrandom(iStim),'ticklength')*4);
else
    set(ax_avpupil_stims_nonrandom(iStim),'yticklabels',[],'box','off','tickdir','out','xtick',[cframespre cframesstim],...
   'xticklabel',[0 5],'XColor', 'none','ticklength',get(ax_avpupil_stims_nonrandom(iStim),'ticklength')*4);
end

axes(ax_avpupil_stims_random(iStim));
clear x
clear y
clear errBar
x=1:size(diam_epochs_random,1);
...
set(ax_avpupil_stims_random(iStim),'box','off','tickdir','out','xtick',[cframespre cframesstim],...
   'xticklabel',[0 5],'XColor', 'none','ticklength',get(ax_avpupil_stims_random(iStim),'ticklength')*4);
else
    set(ax_avpupil_stims_random(iStim),'yticklabels',[],'box','off','tickdir','out','xtick',[cframespre cframesstim],...
   'xticklabel',[0 5],'XColor', 'none','ticklength',get(ax_avpupil_stims_random(iStim),'ticklength')*4);
end
