...
diameter_baseline_stims{iAn}=squeeze(nanmean(diameter_stim{iAn}(30:60,:,:),2));
clear X
...
for istims=1:size(diameter_av_epochs{iAn},2)
    ...
    clear tmp4
    tmp4=diameter_baseline_stims{iAn}(:,istims);
    diameter_av_baseline_stims{iAn}(:,istims) = nanmean(tmp4)
    
    pupil_delta_stims{iAn}(:,istims)=(diameter_av_epochs_stims{iAn}(:,istims)-diameter_av_baseline_stims{iAn}(:,istims))/diameter_av_baseline_stims{iAn}(:,istims)
...
%%
diameter_input=pupil_delta_stims;
diameter_input_array=cell2mat(diameter_input');

diameter_blanks_input=diameter_av_epochs_blanks;
diameter_stims_input=pupil_delta_stims;
%%
clear diameter_mean
...
diam_stimulus0deg=diameter_input_array(:,stim0deg);
diam_stimulus90deg=diameter_input_array(:,stim90deg);
diam_stimulus180deg=diameter_input_array(:,stim180deg);
diam_stimulus270deg=diameter_input_array(:,stim270deg);
...
%%
pupil_animals_av_cell=[av_diam_stimulus0deg(1), av_diam_stimulus90deg(1), av_diam_stimulus180deg(1), av_diam_stimulus270deg(1)];
pupil_animals_cell=[diam_stimulus0deg(:,1), diam_stimulus90deg(:,1), diam_stimulus180deg(:,1), diam_stimulus270deg(:,1)];
...
print(gcf,'-dpdf',[filepathanalysis, 'Figure1E_average_raw_diam_relative_delta_pupil_SEM_animals_STIM_RANDOMIZED.pdf']);
