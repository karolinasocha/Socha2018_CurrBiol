...
%data_in=pupil_data.zscored_data
data_in=pupil_data.raw_relative_diam_delta

pupil_average_data=nan(length(pupil_data.sessions_id),size(data_in,2)-1);
j=0
...
for isess=1:length(indecies_session)
    ...
    pupil_average_data(j,:)=[animals_id(iAn),session_list(isess), nanmean(tmp(:,4:end),1)];
end
...
data_in=pupil_data.raw_relative_diam_delta % all trials
add_val=3;    
