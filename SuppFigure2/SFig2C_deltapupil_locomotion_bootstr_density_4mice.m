...
newdir2=new_pupil_data.newdir;
stimulus2=new_pupil_data.stimulus;
velocity2=new_pupil_data.velocity;
expt12=new_pupil_data.expt;
expt22=new_pupil_data.expt2;

diameter_data2=new_pupil_data.diameter
diameter_data2{10}=diameter_data2{10}';
raw_diam_stims_delta=new_pupil_data.raw_diam_stims_delta;
session_id2=new_pupil_data.sessions_id;
animal_id2=new_pupil_data.animal_id;

av_raw_diam_diff_relative_stimulation2=new_pupil_data.av_raw_diam_diff_relative_stimulation
trials_raw_diam_diff_relative_stimulation2=new_pupil_data.trials_raw_diam_diff_relative_stimulation
%%
%%

for iAn=1:size(newdir2,1);
    clear av
    clear tmp1
    stims{iAn}=expt22{iAn}.frames.stims;  
    [av tmp1]=tcEpochAverage2(diameter_data2{iAn},stims{iAn});
    trials_pupil_stims2{iAn}=tmp1;
clear tmp_file
clear tmp_file
...
array_data_pupil2{iAn}=tmp_file;
matrix_data_pupil2{iAn}=tmp_file(:,:);
end
...
trials_pupil_stims_baseline2=squeeze(nanmean(array_data_pupil2{iAn}(frame_starts:frame_ends,:,:),1));
trials_pupil_stims_offset2=squeeze(nanmean(array_data_pupil2{iAn}(end-frame_starts:end,:,:),1));
trials_delta_pupil2{iAn}=(trials_pupil_stims_offset2-trials_pupil_stims_baseline2)./trials_pupil_stims_baseline2;

trials_pupil_stims_end2{iAn}=trials_pupil_stims_offset2;
end
%% calculate fraction_running
for iAn=1:length(velocity2)
prct_run_all(iAn)=length(find(velocity2{iAn}>0.05))/length((velocity2{iAn}>0.05));
end

%%
%% it needs delta pupil
% trials_raw_diam_diff_relative_stimulation2=new_pupil_data.trials_raw_diam_diff_relative_stimulation;
% raw_relative_diam_delta2=new_pupil_data.raw_relative_diam_delta;

%% SELECT ONLY SESSIONS FROM 4 MICE RUNNING LIKE CRAZY
% selected_loc_animals=[2,4,10,11]
selected_loc_animals=unique(animal_id2(find(prct_run_all>0.65)))

for iAn=1:length(selected_loc_animals)
    clear iAn_index
    iAn
    iAn_index=selected_loc_animals(iAn);
    iAn_index
    indecies{iAn}=find(new_pupil_data.animal_id==iAn_index);   
end
indecies_array=cell2mat(indecies);
indecies_array
%%
% %% select data
newdir=newdir2(indecies_array);
stimulus=stimulus2(indecies_array);
velocity=velocity2(indecies_array);
expt=expt12(indecies_array);
expt2=expt22(indecies_array);
diameter_data=diameter_data2(indecies_array);
session_id=session_id2(indecies_array);
animal_id=animal_id2(indecies_array);
trials_pupil_stims_end=trials_pupil_stims_end2(indecies_array)
% trials_raw_diam_diff_relative_stimulation=trials_raw_diam_diff_relative_stimulation2(indecies_array);
% raw_relative_diam_delta=raw_relative_diam_delta2(indecies_array);
% trials_delta_pupil=trials_raw_diam_diff_relative_stimulation2(indecies_array);

%% calculate fraction_running
for iAn=1:length(velocity)
prct_run(iAn)=length(find(velocity{iAn}>0.05))/length((velocity{iAn}>0.05));
end
...
%         stims{iAn}=expt2{iAn}.frames.stims;
%         tc_pupil{iAn}=diameter_data{iAn};
%         [tc_response_epochs{iAn} tc_trial_response_epochs{iAn}]=tcEpochAverage2(tc_pupil{iAn},epochs{iAn});
%         [tc_response_stims{iAn} tc_trial_response_stims{iAn}]=tcEpochAverage2(tc_pupil{iAn},stims{iAn});    
...
%%
figure
vXEdge = linspace(0,110,60);
vYEdge = linspace(0,110,60);
...
set(gca,'ytick',[0:50:100],'xtick',[0:50:100],'yticklabel',[0:.5:1.5],'xticklabels',[0:.5:1.5],'tickdir','out','box','off','tickdir','out',...
...
print(gcf,'-dpdf',[filepathanalysis, 'FigureSuppl2Bcorrect_resampling_plot_density_locomotion_bootstrapped_4mice.pdf']);
...
% caxis([0 0.0025])
% 
% set(gca,'ytick',[0:50:150],'xtick',[0:50:150],'yticklabel',[0:.5:1],'xticklabel',[0:.5:1],'tickdir','out','box','off','tickdir','out',...
% 'layer','top','color','none','fontsize',14,'ticklength',get(gca,'ticklength')*4)       
% 
% set(gcf,'paperunits','centimeters','papersize' ,[30,30],'color','w','paperposition',[0,0,30,30],'inverthardcopy','off')
% filepathanalysis=['G:\mousebox\code\mouselab\users\karolina\FiguresPaper2023\Figure3\']; 
% %print(gcf,'-dpdf',[filepathanalysis, 'FigureSuppl2Bcorrect_resampling_plot_density_locomotion_bootstr_4mice.pdf']);
% 
