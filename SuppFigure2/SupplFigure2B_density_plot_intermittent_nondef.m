...
ddata_temporal_nondef2=nan([length(newdir), n_pairs, 10]);
ddata_nasal_nondef2=nan([length(newdir), n_pairs,10]);
...
for iAn=1:length(newdir)
       for iistim=1:n_pairs
        ...
        clear nasal_trials_nondef
        clear temporal_trials_nondef
        
        nasal_trials_nondef = find(stim_deg_nondef{iAn}==nasal_istim);
        temporal_trials_nondef= find(stim_deg_nondef{iAn}==temporal_istim);
        
        clear tmp_temporal
        clear tmp_nasal
        
        tmp_nasal=nanmean(av_nondef_eye_trials{iAn}(nasal_trials_nondef,nasal_trials_nondef)',1);;
        tmp_temporal=nanmean(av_nondef_eye_trials{iAn}(temporal_trials_nondef,temporal_trials_nondef)',1);

        ddata_temporal_nondef2(iAn,iistim,1:length(tmp_temporal))=tmp_temporal;
        ddata_nasal_nondef2(iAn,iistim,1:length(tmp_nasal))=tmp_nasal;
...
% try
% [p_original_signrank, ~, stats_original] = signrank(ddata_nasal_nondef2(:),ddata_temporal_nondef2(:),'Tail','right');
% stats_original_signrank = stats_original.signedrank;
% end
[p_original_ranksum, ~, stats_original_ranksum] = ranksum(rmmissing(ddata_nasal_nondef2(:)),rmmissing(ddata_temporal_nondef2(:)),'Tail','right');

[~, p_original_ttest2, ~] = ttest2(rmmissing(ddata_nasal_nondef2(:)),rmmissing(ddata_temporal_nondef2(:)),'Tail','right');

[~, p_original_kstest2, stat_original_kstest2]=kstest2(rmmissing(ddata_nasal_nondef2(:)),rmmissing(ddata_temporal_nondef2(:)),'Tail','larger');
...
for iruns=1:n_runs
    ...
    animals_rand_selections(iruns, :) = animal_subset;
       ...
       clear nasal_trials_nondef
       clear temporal_trials_nondef
       clear ddata_temporal_nondef
       clear ddata_nasal_nondef
...
%        chosen_session=animal_session_indexes(random_session(1));
       ...
       for iistim=1:n_pairs
        ...
        nasal_trials_nondef{iAn} = find(stim_deg_nondef{chosen_session}==nasal_istim);
        temporal_trials_nondef{iAn}= find(stim_deg_nondef{chosen_session}==temporal_istim);
        
        ddata_temporal_nondef{iAn}=nanmean(av_nondef_eye_trials{chosen_session}(temporal_trials_nondef{iAn},temporal_trials_nondef{iAn})',1);
        ddata_nasal_nondef{iAn}=nanmean(av_nondef_eye_trials{chosen_session}(nasal_trials_nondef{iAn},nasal_trials_nondef{iAn})',1);
        % cell2mat((stim_deg_still))'
        
        tt1(iAn)=size(ddata_temporal_nondef{iAn},2);
        tt2(iAn)=size(ddata_nasal_nondef{iAn},2);
...
%             tt1(iAn),nasal_istim,tt2(iAn),temporal_istim));
        if tt2(iAn)>0 & tt1(iAn)>0
            ...
            for iresample = 1:n_sampled_trials
                ...
                nasaldirrawdatadistribution=ddata_nasal_nondef{iAn};
                temporaldirrawdatadistribution=ddata_temporal_nondef{iAn};
...
% kstest2 mean bootstrapping p-value:  9.86e-01
% ttest2 mean bootstrapping p-value:  5.30e-04 p=0.0005
% ranksum mean bootstrapping p-value:  4.02e-04 p=0.0004
% signrank mean bootstrapping p-value:  4.16e-41
%%
prctile_tresh=50;
