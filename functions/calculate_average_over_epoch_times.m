function [av_eye_stims_reordered_onset, trials_eye_stims_reordered_onset,...
    ...
    av_eye_stims_reordered_entire, trials_eye_stims_reordered_entire]= calculate_average_over_epoch_times(expt2, av_eye_stims_reordered, trials_eye_stims_reordered, nsec_starts, nsec_ends)

frameRate=expt2.frameRate;
frame_starts=round(0.5*frameRate)
frame_ends=round(1*frameRate)

frame_starts_middle=round(nsec_starts*frameRate)
frame_ends_middle=round(nsec_ends*frameRate)
...
av_eye_stims_reordered_middle=squeeze(nanmean(av_eye_stims_reordered(frame_starts_middle:frame_ends_middle, :),1));
trials_eye_stims_reordered_middle=squeeze(nanmean(trials_eye_stims_reordered(frame_starts_middle:frame_ends_middle,:,:,:),1));
