...
for iruns=1:nruns
    clear data_in
    permuted_pupil_data_run_average_allmice{iruns}=nanmean(permuted_pupil_data_run{iruns}(:,4:end));
...
for iruns=1:nruns
    ...
    pval_sig_now=nan(nconditions);
    mat_sig_now=nan(nconditions);    
    for ipairs=1:length(pairs_both_sides)
        
        pair_now=pairs_both_sides(ipairs,:);
        stim01=pair_now(1);
        stim02=pair_now(2);
    
        data1=data_all{stim01};
        data2=data_all{stim02};
        
        % ks test
        [~,ks2pval(ipairs),ks2stat(ipairs)]=kstest2(rmmissing(data1(:)),rmmissing(data2(:)),'tail','larger');
        % signrank test
        clear signrankstat_tmp
        [signrankpval(ipairs),~,signrankstat_tmp]=signrank(data1(:),data2(:),'Tail','right');
        %signrankstat(ipair)=signrankstat_tmp.zval;
        
        % ttest2 test 
        clear ttest2stat_tmp
        [~,ttest2pval(ipairs),~, ttest2stat_tmp]=ttest2(rmmissing(data1(:)),rmmissing(data2(:)),'Tail','right');
        
        ttest2stat(ipairs)=ttest2stat_tmp.tstat;
        
        clear ranksumstat_tmp
        [ranksumpval(ipairs),~,ranksumstat_tmp]=ranksum(rmmissing(data1(:)),rmmissing(data2(:)),'Tail','right');

        pval_sig_now(stim01, stim02)=ttest2pval(ipairs);
        
        if sum(pval_sig_now(stim01, stim02)>=0.05)==1
            mat_sig_now(stim01, stim02)=2;
        elseif sum(pval_sig_now(stim01, stim02)<0.05 & pval_sig_now(stim01, stim02)>=0.01)==1
            mat_sig_now(stim01, stim02)=4;
        elseif sum(pval_sig_now(stim01, stim02)<0.01 & pval_sig_now(stim01, stim02)>=0.001)==1
            mat_sig_now(stim01, stim02)=6;            
        elseif sum(pval_sig_now(stim01, stim02)<0.001 & pval_sig_now(stim01, stim02)>=0.0001)==1
            mat_sig_now(stim01, stim02)=8;
        elseif sum(pval_sig_now(stim01, stim02)<0.0001)==1
            mat_sig_now(stim01, stim02)=10;          
        end
    ...
    mat_sig=mat_sig_now;
    mat_sig_individual{iruns}=mat_sig;
    pval_sig_individual{iruns}=pval_sig_now;
%     figure(1)
%     clf
%     [XX,YY]=meshgrid([1:ngroup+1]-0.5,[1:ngroup+1]-0.5);
%     hp=pcolor(XX,YY,padarray(mat_sig,[1 1],0,'post'));
%     set(hp,'edgecolor','w');
%     colorspace=brewermap(10,'Blues');
%     colormap(colorspace)
%     set(gca,'clim',[0 10]);
%     axis square ij
%     box on
...
%%
pval_mean_array=nan(nconditions);   
pval_median_array=nan(nconditions);
mat_sig_mean=nan(nconditions);
for ipairs=1:length(pairs_both_sides)
    ...
    for iruns=1:nruns
        clear pval_tmp;
        pval_tmp=pval_sig_individual{iruns}(stim01,stim02);
        pval_all=[pval_all, pval_tmp];
    end
    
    pval_condition{ipairs}=pval_all;
    pval_condition_mean{ipairs}=nanmean(pval_all);
    pval_condition_median{ipairs}=nanmedian(pval_all);
    pval_median_array(stim01,stim02)=nanmedian(pval_all);
    pval_mean_array(stim01,stim02)=nanmean(pval_all);
    
        if sum(pval_mean_array(stim01, stim02)>=0.05)==1
            mat_sig_mean(stim01, stim02)=2;
        elseif sum(pval_mean_array(stim01, stim02)<0.05 & pval_mean_array(stim01, stim02)>=0.01)==1
            mat_sig_mean(stim01, stim02)=4;
        elseif sum(pval_mean_array(stim01, stim02)<0.01 & pval_mean_array(stim01, stim02)>=0.001)==1
            mat_sig_mean(stim01, stim02)=6;            
        elseif sum(pval_mean_array(stim01, stim02)<0.001)==1
            mat_sig_mean(stim01, stim02)=8;       
        end
end
%%
...
%%
statistical_test_chosen=ks2pval_bootstrap;
