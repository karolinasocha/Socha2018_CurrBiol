...
clear trace_animals
for iAn=1:13
trace_animals(iAn,:,:)=squeeze(nanmean(eye_trace_alltrials_sameanimal{iAn}(:,1:10,:),1));

trials_animals_nasal{iAn}=squeeze(nanmean(eye_trace_alltrials_sameanimal{iAn}(:,1:10,order_nasal),1));
trials_animals_temporal{iAn}=squeeze(nanmean(eye_trace_alltrials_sameanimal{iAn}(:,1:10,order_temporal),1));

end
%%
for iAn=1:13
    trials_animals_nasal_all(iAn,:,:)=trials_animals_nasal{iAn};
    trials_animals_temporal_all(iAn,:,:)=trials_animals_temporal{iAn};
end

avearge_trials_animals_nasal_all=squeeze(nanmean(trials_animals_nasal_all,1));
avearge_trials_animals_temporal_all=squeeze(nanmean(trials_animals_temporal_all,1));
average_trace_animals=squeeze(nanmean(trace_animals,1));
size(average_trace_animals)
%%
...
tmp_temporal=avearge_trials_animals_temporal_all(ntrials,:);
tmp_nasal=avearge_trials_animals_nasal_all(ntrials,:);
