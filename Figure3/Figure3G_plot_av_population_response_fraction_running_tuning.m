...
cdat_ordered_stims{iAn}=cdat_stims(:,:,:,order);
cdat_ordered_stims_mean{iAn}=nanmean(cdat_stims(:,:,:,order),2);
%cdat_ordered_stims_mean{iAn}=nanmax(cdat_stims(:,:,:,order),2);
% --------------------------------
% Sample data
cdat_stims_ttmp = cdat_ordered_stims{iAn};  % Replace with your actual data

% Get the size of the array
dims = size(cdat_stims_ttmp);

% Initialize a variable to store the maximum values
max_values = zeros(dims(1), 1, dims(3), dims(4));  % Neurons x Trials x Directions

% Iterate through neurons, trials, and directions
for neuron = 1:dims(1)
    for trial = 1:dims(3)
        for direction = 1:dims(4)
            % Extract calcium trace for the current neuron, trial, and direction
            calcium_trace = cdat_stims(neuron, :, trial, direction);
            
            % Find the maximum value within the calcium trace
            max_values(neuron, 1, trial, direction) = max(calcium_trace);
        end
    end
end

cdat_ordered_stims_max{iAn}=max_values;
% ---------------------------------
...
cdat_ordered_permuted_stims_mean{iAn}=permute(cdat_ordered_stims_mean{iAn}(:,:,:,1:end-1),[3,1,2,4]);
cdat_ordered_permuted_stims_max{iAn}=permute(cdat_ordered_stims_max{iAn}(:,:,:,1:end-1),[3,1,2,4]);
...
for iAn=1:size(newdir,2);
    ...
    array_tmp_stims{iAn}(:,itrials)=squeeze(nanmean(cdat_ordered_permuted_stims_mean{iAn}(itrials,:,:),2));
    array_tmp_stims_max{iAn}(:,itrials)=squeeze(nanmean(cdat_ordered_permuted_stims_max{iAn}(itrials,:,:),2));
...
calcium_response_array_mean=cell2mat(array_tmp_stims);
calcium_response_array_max=cell2mat(array_tmp_stims_max);
...
calcium_response_array_ord_mean=calcium_response_array_mean(:,ord);
calcium_response_array_ord_max=calcium_response_array_max(:,ord);
...
%%
%% PLOT TUNNING RESPONSE MEAN
%calcium_response_array_ord_mean
...
for iStim =1:6
    ...
    for i=1:length(animal_list_tmp)
        ...
        tmp_calcium=calcium_response_array_ord_mean(:,start_number(iStim):end_number(iStim));
        mean_animal_sel_calcium{i}=nanmean(tmp_calcium(:,tmp_index),2);
    end

    mean_session_array=cell2mat(mean_animal_sel_calcium);

    mean_session_array_nasal=mean_session_array(order_nasal,:);
    mean_session_array_temporal=mean_session_array(order_temporal,:);
    [p_original_signrank(iStim), ~, stats_original] = signrank(mean_session_array_nasal(:),mean_session_array_temporal(:),'Tail','right');

    [p_original_ranksum(iStim), ~, stats_original_ranksum] = ranksum(rmmissing(mean_session_array_nasal(:)),rmmissing(mean_session_array_temporal(:)),'Tail','right');

    [~, p_original_ttest2(iStim), ~] = ttest2(rmmissing(mean_session_array_nasal(:)),rmmissing(mean_session_array_temporal(:)),'Tail','right');
    ...
    ylim([0 120]);
    hold on
    plot([0 size(calcium_response_array_ord_mean,1)],[50 50],'--k');
    hold on
    plot([13 13],[0 50],'-k');
    axis off
end
...
% print(gcf,'-dpdf',[filepathanalysis, 'Figure3G_plot_avdFF_calcium_response_population_fractionrunning_tuning.pdf']);
% print(gcf,'-dpdf',[filepathanalysis, 'Figure3F_plot_avdFF_calcium_response_population_fractionrunning.svg']);

% p_original_ranksum = [0.0051    0.0015    0.0003    0.0062    0.0003   0.1193] ;
% p_original_signrank = [0.0000    0.0000    0.0000    0.0000    0.0000   0.0224]; 
% p_original_ttest2 =[0.0077    0.0010    0.0001    0.0045    0.0005    0.1593];
%%
%% PLOT TUNNING RESPONSE MAX VALUE BUT NO EFFECT
%calcium_response_array_ord_mean
figure
clf
...
for iStim =1:6
    ...
    for i=1:length(animal_list_tmp)
        tmp_index=find(animal_tmp==animal_list_tmp(i));
        tmp_calcium=calcium_response_array_ord_max(:,start_number(iStim):end_number(iStim));
        mean_animal_sel_calcium{i}=nanmean(tmp_calcium(:,tmp_index),2);
    end
