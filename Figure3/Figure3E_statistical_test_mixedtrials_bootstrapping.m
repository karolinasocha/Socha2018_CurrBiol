...
clear av_loc_eye_trials
    ...
    for iAn = 1:size(resp_nondef_resp_trials,2)
...
for iAn=1:length(newdir)
       for iistim=1:n_pairs
        ...
        ddata_temporal_nondef(iAn,iistim,1:length(tmp_temporal))=tmp_temporal;
        ddata_nasal_nondef(iAn,iistim,1:length(tmp_nasal))=tmp_nasal;
        % cell2mat((stim_deg_still))'
                
       end
end

[p_original_signrank, ~, stats_original] = signrank(ddata_nasal_nondef(:),ddata_temporal_nondef(:),'Tail','right');
stats_original_signrank = stats_original.signedrank;

[~, p_original_ttest2, ~] = ttest2(ddata_nasal_nondef(:),ddata_temporal_nondef(:),'Tail','right');
...
for iruns=1:n_runs
    ...
    for iAn=1:length(animal_subset)
       ...
       for iistim=1:n_pairs
        ...
        ddata_temporal_nodef{iAn}=nanmean(av_nondef_eye_trials{chosen_session}(temporal_trials_nondef{iAn},temporal_trials_nondef{iAn})',1);
...
print(gcf,'-dpdf',[filepathanalysis, 'Figure3Fcorrect_resampling_plot_density_mixed_dF_bootstrapped.pdf']);
