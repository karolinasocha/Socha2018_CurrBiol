...
ntrials= length(intersect(find(stimuli_params(:,1)==0),find(stimuli_params(:,3)==4 )));
stim_duration=5;
prestim_duration=5;
poststim_duration=5;

pupil_trials_stim=nan(length(stim_directions),length(stim_tfs), ntrials,22*stim_duration); % 110 for 5s
pupil_trials_poststim=nan(length(stim_directions),length(stim_tfs), ntrials,22*stim_duration);
pupil_trials_epochs=nan(length(stim_directions),length(stim_tfs), ntrials,110+22*stim_duration); % 220 for 5s
pupil_trials_entire_epochs=nan(length(stim_directions),length(stim_tfs), ntrials,220+22*stim_duration); % 330 for 5s

for istim=1:length(stim_directions)
    for itf = 1:length(stim_tfs)
        
    indx=intersect(find(stimuli_params(:,1)==stim_directions(istim)),find(stimuli_params(:,3)==stim_tfs(itf)));
    ...
    for itrial=1:length(stimOnset_indx)
        ...
        pupil_trials_stim(istim,itf, itrial,1:length(tmp))=tmp;
        
        clear tmp2
        tmp2=pupil_area_zscore(nearestIndex_onsetPre:nearestIndex_onset);
        pupil_trials_blanks(istim,itf, itrial,1:length(tmp2))=tmp2;
        
        clear tmp3
        tmp3=pupil_area_zscore(nearestIndex_offset:nearestIndex_offsetPost);
        pupil_trials_poststim(istim,itf, itrial,1:length(tmp3))=tmp3;
        
        clear tmp4
        tmp4=pupil_area_zscore(nearestIndex_onsetPre:nearestIndex_offset);
        pupil_trials_epochs(istim,itf, itrial,1:length(tmp4))=tmp4;
        
        clear tmp5
        tmp5=pupil_area_zscore(nearestIndex_onsetPre:nearestIndex_offsetPost);
        pupil_trials_entire_epochs(istim,itf, itrial,1:length(tmp5))=tmp5;
