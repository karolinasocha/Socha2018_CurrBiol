...
sessions={'run01_140753_5sf-4dir-tf4-SinRand5s', 'run01_152943_5sf-4dir-tf4-SinRand5s','run01_164309_5sf-4dir-tf4-SinRand5s'}
...
for istim = 1:length(stim_directions) % Assuming this iterates over rows
    for isf = 1:length(stim_sfs) % Assuming this iterates over columns
        % Calculate position
        % Adjust the spacing values accordingly to fit the plots within the figure window
        left = .07 + .1 * (isf - 1); % Adjust horizontal spacing based on column
        bottom = .8 - .15 * (istim - 1); % Adjust vertical spacing based on row
        width = .05; % Fixed width
        height = .1; % Fixed height
        
        % Create axes for the plot
        ax_avpupil_nasal(istim,isf) = axes('position',[left, bottom, width, height],'units','normalized');
    end
end
...
nAn=size(pupil_av_entire_epochs_array,5);
 ...
 for istim=1:length(stim_directions)
    ...
    axes(ax_avpupil_nasal(istim,isf))

    clear pupil_animals_cell
    pupil_animals_cell=squeeze(pupil_av_entire_epochs_array(istim,isf,:,:,:));
%
...
hold on;
fill([x, fliplr(x)], [meanData - semData, fliplr(meanData + semData)], 'k', 'FaceAlpha', 0.8, 'EdgeColor', 'k');
...
title(sprintf('%.fdeg\n%.2fcpd', stim_directions(istim), stim_sfs(isf)));
...
filename=[session,'_averaged_epochs_differentSFs']
...
plot(x, meanData, 'k', 'LineWidth', 2);
hold on;
h=fill([x, fliplr(x)], [meanData - semData, fliplr(meanData + semData)], 'k', 'FaceAlpha', 0.3, 'EdgeColor', 'k');
hold on
